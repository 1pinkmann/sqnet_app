...
contract SQNK is Context, IERC20 {
  ...
  function setExcludedFromTax (address _address, bool _status) public onlyOwner {
    excludedFromTax[_address] = _status;
  }

  function getExcludedFromTax (address _address) public view returns (bool) {
    return excludedFromTax[_address];
  }
  ...
  function getTaxes() public view returns(uint, uint, uint) {
    return (MARKETING_TAX, REWARD_TAX, LIQUIDITY_TAX);
  }
